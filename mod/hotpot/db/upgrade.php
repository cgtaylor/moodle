...
function xmldb_hotpot_upgrade($oldversion) {
    ...
    $newversion = 2010080307;
    if ($oldversion < $newversion) {
        // add new capabilities (mod/hotpot:manage, mod/hotpot:preview, mod/hotpot:view, mod/hotpot:review)
    ...
    if ($oldversion < $newversion) {
        ...
        if ($count = $DB->count_records_sql("SELECT COUNT('x') FROM $from WHERE $where", $params)) {
            $rs = $DB->get_recordset_sql("SELECT $select FROM $from WHERE $where", $params);
        } else {
            $rs = false;
        }

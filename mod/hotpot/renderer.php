...
class mod_hotpot_renderer extends plugin_renderer_base {
    ...
    public function attemptssummary($hotpot)  {
        ...
        if (has_capability('mod/hotpot:deleteattempt', $hotpot->context)) {
            $showselectcolumn = true; // false;
        } else {
            $showselectcolumn = false;
        }

        if (has_capability('mod/hotpot:viewreport', $hotpot->context)) {
            // teacher
            $has_capability_review = true;
            $resumetab = 'preview';
        } else if (has_capability('mod/hotpot:view', $hotpot->context)) {
            // student
            $has_capability_review = true;
            $resumetab = 'info';
        } else {
    ...
    public function exitlinks($hotpot)  {
        ...
        if ($hotpot->attempt->status==hotpot::STATUS_COMPLETED) {
            ...
            if ($cm = $hotpot->get_cm('exit')) {
                $url = new moodle_url('/mod/'.$cm->mod.'/view.php', array('id' => $cm->cm));
                $table->data[] = new html_table_row(array(

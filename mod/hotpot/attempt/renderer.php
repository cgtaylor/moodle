...
class mod_hotpot_attempt_renderer extends mod_hotpot_renderer {
    ...
    function print_embed_object_page()  {
        ...
        $src = new moodle_url('/mod/hotpot/iframe.js');
    ...
    function fix_relativeurls($str=null)  {
        ...
        $replace = '$this->convert_url_relative("'.$this->hotpot->source->baseurl.'","'.$this->hotpot->sourcefile.'","\\1","\\7","\\8")';
        ...
        $replace = '"\\1".$this->convert_urls_stylesheets("'.$this->hotpot->source->baseurl.'","'.$this->hotpot->source->filepath.'","\\2")."\\3"';
        $this->headcontent = preg_replace($search, $replace, $this->headcontent);
        $this->bodycontent = preg_replace($search, $replace, $this->bodycontent);

        // replace relative URLs in <a ... onclick="window.open('...')...">...</a>
        $search = '/'.'(?<='.'onclick="'."window.open\('".')'."([^']*)".'(?='."'\);return false;".'")'.'/ise';
        $replace = '$this->convert_url("'.$this->hotpot->source->baseurl.'","'.$this->hotpot->source->filepath.'","\\1")';
        $this->bodycontent = preg_replace($search, $replace, $this->bodycontent);
    }
    ...
    function convert_url($baseurl, $sourcefile, $url, $quote="'")  {
        ...
        $replace_pairs = array(
            ...
            '%coursepage%' => $CFG->wwwroot.'/course/view.php?id='.$this->hotpot->cm->id,
            '%coursefiles%' => $CFG->wwwroot.'/'.$file_php.'/'.$this->hotpot->cm->id
        );

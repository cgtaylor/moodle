...
class mod_hotpot_attempt_hp_renderer extends mod_hotpot_attempt_renderer {
   ...
   function set_bodycontent()  {
    ...
    public function fix_relativeurls($str=null) {
        ...
        $replace = '$this->convert_urls_preloadimages("'.$this->hotpot->source->baseurl.'","'.$this->hotpot->source->filepath.'","\\1")';
     ...
     * @return xxx
     */
    public function convert_urls_preloadimages($baseurl, $sourcefile, $urls, $quote="'")  {
        if ($quote) {
            // fix quotes escaped by preg_replace
     ...
     * @return xxx
     */
    public function convert_url_navbutton($baseurl, $sourcefile, $url, $quote="'")  {
        global $CFG, $DB;
        if ($quote) {
            // fix quotes escaped by preg_replace
        ...
        if (strpos($url, $baseurl.'/')===0) {
            $sourcefile = $this->hotpot->source->xml_locate_file(substr($url, strlen($baseurl)));

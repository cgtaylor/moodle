...
class mod_hotpot_report_renderer extends mod_hotpot_renderer {
    ...
    public function reportcontent()  {
        ...
        // check capabilities
        if (has_capability('mod/hotpot:viewreport', $this->hotpot->context)) {
            $userid = 0; // all users
        } else {
        ...
        if (! $this->hotpot->can_deleteattempt()) {
            // remove the select column from students view
            $i = array_search('selectattempt', $tablecolumns);
            if (is_numeric($i)) {
    ...
    function display_filters($baseurl) {
        if (count($this->filterfields) && $this->hotpot->can_viewreport()) {

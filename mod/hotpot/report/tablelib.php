...
class hotpot_report_table extends table_sql {
    ...
    function wrap_html_start() {
        ...
        // check this table has a "selectattempt" column
        if (! $this->has_column('selectattempt')) {
            return false;
        }

        // check user can delete attempts
        if (! $this->output->hotpot->can_deleteattempt()) {
            return false;
        }
    ...
    function wrap_html_finish() {
        ...
        $params = array('type'=>'submit', 'onclick'=>"return confirm('$confirm')",
                        'name'=>'delete', 'value'=>get_string('deleteattempts', 'hotpot'));
    ...
    /**
     * header_selectattempt
     *
     * @return xxx
     */
    function header_selectattempt()  {
        return '';
    }
     ...
     * @return xxx
     */
    function col_selectattempt($row)  {
        $params = array('type'=>'checkbox', 'name'=>'selectattempt[]', 'value'=>$row->id);
    ...
    function format_review_link($text, $row)  {
        if (strlen($text) && $this->output->hotpot->can_review()) {

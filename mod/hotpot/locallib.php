...
class hotpot {
    ...
    const ACTIVITY_COURSE_QUIZPORT  = -3;
    const ACTIVITY_SECTION_QUIZPORT = -4;
    ...
    /** @var boolean cache for hotpot:deleteattempt capability */
    public $candeleteattempt;

    /** @var boolean cache for hotpot:preview capability */
    public $canpreview;

    /** @var boolean cache for hotpot:review capability */
    public $canreview;

    /** @var boolean cache for hotpot:view capability */
    public $canview;

    /** @var boolean cache for hotpot:viewreport capability */
    ...
    public static function available_outputformats_list($sourcetype) {
        ...
        if ($sourcetype) {
            ...
            foreach ($classes as $class) {
                ...
                eval('$sourcetypes = '.$class.'::sourcetypes();');
   ...
   public static function load_mediafilter_filter($classname)  {
    ...
    public function get_source() {
        ...
        if (empty($this->source)) {
            ...
            $this->source = new $classname($sourcefile, $this->course->id, $this->sourcelocation);
        }
        return $this->source;
    }
     ...
     * @return xxx
     */
    function can_deleteattempt()  {
        if (is_null($this->candeleteattempt)) {
            $this->candeleteattempt = has_capability('mod/hotpot:deleteattempt', $this->context);
        }
        return $this->candeleteattempt;
    }
     ...
     */
    function can_review()  {
        if (is_null($this->canreview)) {
            $this->canreview = has_capability('mod/hotpot:review', $this->context);
        }
     ...
     * @return xxx
     */
    function can_viewreport()  {
        if (is_null($this->canviewreport)) {
            $this->canviewreport = has_capability('mod/hotpot:viewreport', $this->context);
        }
     ...
     * @param xxx $selected
     */
    public function delete_attempts($selected) {
        global $DB, $USER;

        if (empty($selected) || ! $this->get_attempts()) {
            return; // nothing to do
        }

        if (! has_capability('mod/hotpot:deleteattempt', $this->context)) {
            return; // user does not have permission to delete attempts
        }
        ...
        hotpot_update_grades($this->to_stdclass(), $USER->id);
    }
    ...
    public function require_entrycm()  {
        ...
        $href = new moodle_url('/mod/'.$cm->mod.'/view.php', array('id' => $cm->cm));

        if ($this->entrygrade) {
            if ($grades = grade_get_grades($this->course->id, 'mod', $cm->mod, $cm->instance, $USER->id)) {
                $grade = 0;
    ...
    public function require_moreattempts($shorterror=false)  {
        ...
        if ($shorterror) {
            ...
            $attemptlimitstr = moodle_strtolower(get_string('attemptlimit', 'hotpot'));
    ...
    public function get_cm($type)  {
        ...
        if ($cmid>0) {
            ...
            if ($cmid==self::ACTIVITY_COURSE_QUIZPORT  || $cmid==self::ACTIVITY_SECTION_QUIZPORT) {
                $module = 'hotpot';
            }
            if ($cmid==self::ACTIVITY_SECTION_ANY || $cmid==self::ACTIVITY_SECTION_QUIZPORT) {
                $section = $modinfo[$this->cm->id]->section;
            }
        }
